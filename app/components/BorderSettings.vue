...
const logoImageUrl = computed(() => overrides.value.logoImageUrl ?? resolvedConfig.value?.logoImageUrl ?? '')
const defaultModelFontSize = computed(() =>
  overrides.value.modelFontSize ?? resolvedConfig.value?.modelFontSize ?? 20
)
const logoWidth = computed({
  get: () => overrides.value.logoWidth ?? resolvedConfig.value?.logoWidth ?? defaultModelFontSize.value * 5,
  set: v => update('logoWidth', v),
})
const logoHeight = computed({
  get: () => overrides.value.logoHeight ?? resolvedConfig.value?.logoHeight ?? Math.round(defaultModelFontSize.value * 1.4),
...
const socialRatios: { label: string; value: '1:1' | '4:5' | '3:4' | '1.91:1' | '5:4' | '4:3' | '1:1.91' }[] = [
  { label: '1:1', value: '1:1' },
  { label: '4:5', value: '4:5' },
...
<template>
  <div v-if="resolvedConfig" class="flex flex-col gap-5 text-sm text-nord-4">
    ...
    <div class="flex items-center justify-between">
      <span class="text-nord-5 font-medium">背景颜色</span>
    ...
    <div class="flex flex-col gap-3">
      <span class="text-nord-5 font-medium border-b border-nord-2 pb-1">边框</span>
      <div class="flex items-center justify-between gap-3">
        <label class="flex-1">
          <span class="text-xs text-nord-4 mb-1 block">粗细 {{ borderWidth }}px</span>
          <input type="range" min="0" max="40" v-model.number="borderWidth"
      ...
      <div>
        <span class="text-xs text-nord-4 mb-1 block">圆角 {{ borderRadius }}px</span>
        <input type="range" min="0" max="60" v-model.number="borderRadius"
          class="w-full accent-nord-8" />
      </div>
    </div>
    ...
    <!-- 留白 -->
    <div class="flex flex-col gap-3">
      <span class="text-nord-5 font-medium border-b border-nord-2 pb-1">留白</span>
      <div>
        <span class="text-xs text-nord-4 mb-1 block">四边均等 {{ padding }}px</span>
        <input type="range" min="0" max="200" v-model.number="padding"
          class="w-full accent-nord-8" />
      </div>
      <div>
        <span class="text-xs text-nord-4 mb-1 block">底部额外 {{ paddingBottom }}px</span>
        <input type="range" min="0" max="300" v-model.number="paddingBottom"
          class="w-full accent-nord-8" />
      </div>
    </div>
    ...
    <div class="flex flex-col gap-2">
      <span class="text-nord-5 font-medium border-b border-nord-2 pb-1">画布比例</span>
      <div class="flex rounded-lg overflow-hidden border border-nord-3">
        <button
          ...
          :class="canvasMode !== 'social' ? 'bg-nord-8 text-nord-0 font-medium' : 'bg-nord-2 text-nord-4 hover:bg-nord-3'">
          原图
        </button>
        <button
      ...
      <button v-if="canvasMode === 'social'" @click="invertSocialRatio"
        ...
        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h5M20 20v-5h-5M5.07 9A8 8 0 019 4.07M19.93 15A8 8 0 0115 19.93" />
        </svg>
        反转比例
      </button>
      <p class="text-[10px] text-nord-4/70">{{ canvasMode === 'social' ? socialDimsText : '跟随原图比例' }}</p>
    </div>
    ...
    <div class="flex flex-col gap-3">
      <div class="flex items-center justify-between border-b border-nord-2 pb-1">
        ...
        <label class="flex items-center gap-2 cursor-pointer">
          ...
          <span class="text-xs">显示</span>
        </label>
      </div>

      <!-- 位置（始终可用：有 logo 时定位 logo，无 logo 时定位参数） -->
      <div class="flex items-center justify-between">
        <span class="text-xs text-nord-4">位置</span>
        <div class="flex rounded-lg overflow-hidden border border-nord-3">
          <button v-for="pos in ['left', 'center', 'right']" :key="pos"
            @click="logoPosition = pos as 'left' | 'center' | 'right'"
            class="px-3 py-1 text-xs transition-colors"
            :class="logoPosition === pos ? 'bg-nord-8 text-nord-0 font-medium' : 'bg-nord-2 text-nord-4 hover:bg-nord-3'">
            {{ pos === 'left' ? '左' : pos === 'center' ? '中' : '右' }}
          </button>
        </div>
      </div>
      <p v-if="!showLogo" class="text-[10px] text-nord-4/70">未显示 Logo，位置将作用于参数</p>

      <template v-if="showLogo">
        <!-- Logo 图片上传 -->
        <div class="flex flex-col gap-2">
          <span class="text-xs text-nord-4">Logo 图片（优先于文字）</span>
          <div v-if="logoImageUrl" class="relative flex items-center gap-2 p-2 bg-nord-2 rounded-lg">
            <img :src="logoImageUrl" class="h-8 object-contain max-w-[120px] rounded" />
            <button @click="clearLogoImage"
              class="ml-auto text-xs text-nord-11 hover:text-red-400 transition-colors px-2 py-1 rounded hover:bg-nord-3">
              移除
            </button>
          </div>
          <button v-else @click="logoImageInput?.click()"
          ...
          <input ref="logoImageInput" type="file" accept="image/*" class="hidden" @change="onLogoImageChange" />

          <!-- Logo 图片大小 -->
          <template v-if="logoImageUrl">
            <div>
              <span class="text-xs text-nord-4 mb-1 block">图片宽度 {{ logoWidth }}px</span>
              <input type="range" min="20" max="400" v-model.number="logoWidth" class="w-full accent-nord-8" />
            </div>
            <div>
              <span class="text-xs text-nord-4 mb-1 block">图片高度 {{ logoHeight }}px</span>
              <input type="range" min="10" max="200" v-model.number="logoHeight" class="w-full accent-nord-8" />
            </div>
          </template>
        </div>
        ...
        <!-- Logo 文字（备用） -->
        <div>
          <span class="text-xs text-nord-4 mb-1 block">Logo 文字（留空使用相机品牌）</span>
          <input type="text" v-model="logoText" placeholder="e.g. NIKON CORPORATION"
