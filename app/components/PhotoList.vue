...
async function handleFiles(files: FileList | File[]) {
  ...
  for (let i = 0; i < imageFiles.length; i++) {
    ...
    try {
      ...
      console.error('[FantasticFrame] 导入失败:', file.name, err)
    }
  ...
  <div class="w-full flex items-center gap-2 h-full">
    ...
    >
      <template v-if="!isImporting">
        <svg class="w-6 h-6 text-nord-3 group-hover:text-nord-8 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
        </svg>
        <span class="text-[10px] text-nord-3 group-hover:text-nord-8 transition-colors">添加</span>
      </template>
      <template v-else>
        <!-- 导入进度 -->
        <div class="flex flex-col items-center justify-center gap-1">
          ...
          <span class="text-[10px] text-nord-8">{{ importProgress.current }}/{{ importProgress.total }}</span>
        </div>
      </template>
    </button>

    <!-- 照片列表 -->
    <div class="flex-1 overflow-x-auto flex gap-3 py-2 scrollbar-thin scrollbar-thumb-nord-2 scrollbar-track-nord-0">
      <div
      ...
      >
        ...
        <!-- Filename Overlay -->
        <div class="absolute bottom-0 left-0 right-0 bg-nord-0/80 backdrop-blur-sm text-nord-6 text-[10px] px-1 py-0.5 truncate text-center">
          {{ photo.fileName }}
        </div>
      </div>

      <div v-if="photoStore.photos.length === 0" class="flex items-center justify-center text-nord-4 text-sm min-w-[160px]">
        暂无照片，点击左侧添加
      </div>
    </div>
